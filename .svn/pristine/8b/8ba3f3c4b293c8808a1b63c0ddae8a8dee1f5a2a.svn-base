...
gnome2_src_prepare() {
	...
	# shouldn't occur.
	has ${EAPI:-0} 4 4-python 5 5-progress && gnome2_omf_fix
...
gnome2_src_install() {
	...
	if has ${EAPI:-0} 4 4-python 5 5-progress; then
		mkdir -p "${installation_prefix}${sk_tmp_dir}" || die "mkdir failed"
		emake DESTDIR="${GNOME2_DESTDIR:-${D}}" scrollkeeper_localstate_dir="${installation_prefix}${sk_tmp_dir}" "$@" install
	else
		emake DESTDIR="${GNOME2_DESTDIR:-${D}}" "$@" install
		einstalldocs
	fi
	...
	elif has ${EAPI:-0} 5 5-progress; then
		einstalldocs
	else
		local d
