...
if [[ "${PV}" == *_pre* ]]; then
	...
	EGIT_BRANCH="3.6"
	EGIT_COMMIT="58521fdba1657f6553a1ead5cbaa100967a167b3"
else
	MY_PV="${PV%_p*}"
	MY_P="Python-${MY_PV}"
fi

PATCHSET_REVISION="20170716"
...
PYTHON_ABI="${SLOT}"
KEYWORDS="~*"
IUSE="build elibc_uclibc examples gdbm ipv6 +ncurses +readline sqlite +ssl +threads tk wininst +xml"
...
src_prepare() {
	...
	sed -e "/if threading and not any(is_multiprocess_flag(arg) for arg in regrtest_args):/,+1d" -i Tools/scripts/run_tests.py
