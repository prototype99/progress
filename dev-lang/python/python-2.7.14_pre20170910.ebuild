...
if [[ "${PV}" == *_pre* ]]; then
	...
	EGIT_BRANCH="2.7"
	EGIT_COMMIT="1911cf3dd2ae67d600c166ba52872fdcf3e85824"
else
	MY_PV="${PV%_p*}"
	MY_P="Python-${MY_PV}"
fi
...
PYTHON_ABI="${SLOT}"
KEYWORDS="*"
IUSE="-berkdb build elibc_uclibc examples gdbm ipv6 +ncurses +readline sqlite +ssl +threads tk +wide-unicode wininst +xml"
...
src_test() {
	...
	nonfatal emake test EXTRATESTOPTS="-w" < /dev/tty
	local result="$?"
