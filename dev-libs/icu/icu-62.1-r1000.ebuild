...
inherit epatch flag-o-matic multilib-minimal toolchain-funcs versionator
...
src_prepare() {
	epatch "${FILESDIR}/${PN}-62.1-pinCapacity.patch"
...
multilib_src_test() {
	if [[ "${ABI}" == "x86" ]]; then
		...
		sed -e "/case 5: name = \"TestFixedDecimal\";/,/break;/d" -i test/intltest/dcfmapts.cpp
