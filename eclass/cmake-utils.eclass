...
# @SUPPORTED_EAPIS: 5 5-progress 6
...
case ${EAPI} in
	5|5-progress) : ${CMAKE_WARN_UNUSED_CLI:=no} ;;
...
case ${EAPI} in
	5|5-progress|6) inherit eutils multilib ;;
esac
...
_cmake_use_me_now() {
	...
	[[ ${EAPI} =~ ^(5|5-progress)$ ]] || die "${FUNCNAME[1]} is banned in EAPI 6 and later: use -D$1<related_CMake_variable>=\"\$(usex $2)\" instead"
...
_cmake_use_me_now_inverted() {
	...
	if [[ ! ${EAPI} =~ ^(5|5-progress)$ && "${FUNCNAME[1]}" != cmake-utils_use_find_package ]] ; then
...
comment_add_subdirectory() {
	[[ ${EAPI} =~ ^(5|5-progress)$ ]] || die "comment_add_subdirectory is banned in EAPI 6 and later - use cmake_comment_add_subdirectory instead"
...
cmake-utils_src_configure() {
	...
	[[ ${EAPI} =~ ^(5|5-progress)$ ]] && _cmake_cleanup_cmake
	...
	if [[ ${CMAKE_BUILD_TYPE} != Gentoo && ! ${EAPI} =~ ^(5|5-progress)$ ]]; then
		cat >> ${common_config} <<- _EOF_ || die
	...
	local cmakeargs=(
		...
		$([[ ${EAPI} =~ ^(5|5-progress)$ ]] && echo -DCMAKE_INSTALL_DO_STRIP=OFF)
